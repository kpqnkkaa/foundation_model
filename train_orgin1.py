...
def gaze_collate_fn(batch):
    ...
    # [关键修改] 用于验证的原始数据列表，必须与 task_types 一一对应
    val_gt_points = []
    val_gt_inouts = []
    ...
    for item in batch:
        ...
        for ge in ge_list:
            ...
            # [关键] Estimation 样本没有 gaze point GT，填 None 占位
            # 这样列表总长度 = len(task_types)
        ...
        # fol_gaze_front_back.append(gf_fol['gaze_front_back'])
        # fol_gaze_dir_cls.append(gf_fol['gaze_dir_cls'])
    ...
    batch_dict = {
        ...
        # "gaze_front_back": torch.stack(fol_gaze_front_back) if fol_gaze_front_back else None,
        # "gaze_dir_cls": torch.stack(fol_gaze_dir_cls) if fol_gaze_dir_cls else None,
        # 使用构建好的对齐列表
...
def get_args():
    parser = argparse.ArgumentParser(description="Unified Gaze Training")
    ...
    # prompt种类
    parser.add_argument('--prompt_type', type=str, default='bbox', choices=['bbox'])
    
    # 损失权重
    parser.add_argument('--alpha_est_est', type=float, default=2.0)
    parser.add_argument('--alpha_est_fol', type=float, default=0.0)
    parser.add_argument('--alpha_text', type=float, default=0.0)
    parser.add_argument('--alpha_point', type=float, default=5.0)
    parser.add_argument('--alpha_io', type=float, default=0.0)
    parser.add_argument('--alpha_joint', type=float, default=0.0)
    parser.add_argument('--alpha_head_mask', type=float, default=0.0)
    parser.add_argument('--alpha_gaze_dir', type=float, default=0.0)
...
def main():
    ...
    param_groups = [
        {'params': model.estimation_branch.parameters(), 'lr': args.lr}, 
        {'params': model.following_branch.parameters(), 'lr': args.lr},
    ]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Scheduler Step
        scheduler.step(val_fol_l2)
        current_lr = optimizer.param_groups[0]['lr']
        
        # Log
        log_str = f"=== Epoch [{epoch+1}/{args.epochs}] Finished | Train Loss: {train_loss:.4f} | Val Fol L2: {val_fol_l2:.4f} | LR: {current_lr:.2e} ==="
