...
class DINOv2ImageEncoder(nn.Module):
    def __init__(self, model_name="dinov2_vitb14", out_dim=256, freeze_backbone=True):
        """
        DINOv2 Encoder 适配器
        ...
        # 1. 加载 DINOv2 (从 PyTorch Hub)
        # 第一次运行会自动下载权重
        self.backbone = torch.hub.load('facebookresearch/dinov2', model_name)
        
        # 获取 DINOv2 的原始通道数 (Small: 384, Base: 768, Large: 1024)
        self.embed_dim = self.backbone.embed_dim
        
        # 2. 冻结骨干网络 (建议冻结，DINOv2 特征已经很强了，且显存占用大)
        if freeze_backbone:
            for param in self.backbone.parameters():
                param.requires_grad = False
        
        # 3. 投影层: 将 DINOv2 的通道数 (768) 映射到 SAM 需要的 (256)
        # 使用 1x1 卷积作为 Adapter
        self.projector = nn.Sequential(
            nn.Conv2d(self.embed_dim, out_dim, kernel_size=1),
            nn.LayerNorm([out_dim, 32, 32]), # 注意：LayerNorm 通常对 C 做，这里加个 BN 或者 LN 稳定训练
            nn.ReLU()
        )
        # 更稳健的写法：Conv2d -> GroupNorm/BatchNorm -> ReLU
        self.projector = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # DINOv2 forward_features 返回一个字典
        # 我们需要 "x_norm_patchtokens" (Patch Token, 不包含 CLS)
        # 输入必须是 14 的倍数 (448 是 14*32，正好)
        features_dict = self.backbone.forward_features(x)
        patch_tokens = features_dict["x_norm_patchtokens"] # [B, L, D] -> [B, 1024, 768]
        ...
        # [B, L, D] -> [B, D, H, W]
        feat_spatial = patch_tokens.permute(0, 2, 1).reshape(B, self.embed_dim, H, W)
...
class FollowingBranch(nn.Module):
    def __init__(self):
        ...
        self.scene_encoder = SAMImageEncoderReal(self.ckpt_name, model_type, lora_r=lora_rank, img_size=448)
        ...
        self.head_token_proj = nn.Linear(256, 256)
    ...
    def forward(self, scene_img, obs_text_ids, face_bbox, eye_point, target_text_ids=None):
        ...
        pred_gaze_dir = self.gaze_dir_pred(feat_256)        # [B, 1]
        ...
        # head_token_out = tokens_out[:, 1, :] # 第 1 个是 Head (虽然这里没用它做输出，但它参与了 Attention 交互)
        ...
        return {
            ...
            "pred_gaze_dir": pred_gaze_dir    # 与 2D Gaze GT 做 MSE Loss
        }
